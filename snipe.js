...
const ETHEREUM_LOGO = path.join(__dirname, 'ethereum-logo.png');
  ...
  for (const endpoint of RPC_ENDPOINTS) {
    try {
      ...
      const balance = await Promise.race([
        provider.getBalance(ethers.getAddress(publicKeyString)),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 10000))
      ]);
      const ethBalance = Number(ethers.formatEther(balance));
      balanceCache.set(cacheKey, { balance: ethBalance, timestamp: now });
      return ethBalance;
...
async function getEthPriceWithCache() {
  const now = Date.now();
  if (ethPriceCache.price > 0 && (now - ethPriceCache.timestamp < PRICE_CACHE_TTL)) return ethPriceCache.price;
  try {
    const res = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${WETH_ADDRESS}`);
    const data = await res.json();
    if (data.pairs && data.pairs.length) {
      const ethPair = data.pairs.find(p => p.chainId === 'ethereum' && (p.quoteToken?.symbol === 'USDC' || p.quoteToken?.symbol === 'USDT'));
      if (ethPair && ethPair.priceUsd) {
    ...
    if (ethPriceCache.price > 0) return ethPriceCache.price;
    return 0;
  }
}
...
async function getMintDecimals(mintAddress) {
  if (!mintAddress) return 18;
  if (mintDecimalsCache.has(mintAddress)) return mintDecimalsCache.get(mintAddress);
  try {
    const token = new ethers.Contract(mintAddress, ['function decimals() view returns (uint8)'], provider);
    const dec = Number(await token.decimals());
    mintDecimalsCache.set(mintAddress, dec);
    return dec;
  } catch { return 18; }
}

function reethveTradeTokenAmount(trade) {
...
function recomputeTradePnL(tradeHistory) {
  ...
  for (const trade of byTime) {
    ...
    const qty = reethveTradeTokenAmount(trade);
...
bot.use(async (ctx, next) => {
  conethe.log('Update:', ctx.updateType);
  if (ctx.message) conethe.log('Message:', ctx.message.text);
  await next();
});
...
async function loadSessions() {
  try {
    ...
    conethe.log(`✅ Loaded ${userSessions.size} sessions`);
    if (mutatedAny) await saveSessions();
  } catch (err) {
    if (err.code !== 'ENOENT') conethe.error('Load sessions error:', err);
    else conethe.log('No existing sessions file, starting fresh.');
  }
}
...
async function saveSessions() {
  try {
    ...
    conethe.log(`✅ Saved ${userSessions.size} sessions`);
  } catch (err) {
    conethe.error('Save sessions error:', err);
  }
}
...
function createWallet() {
  const wallet = ethers.HDNodeWallet.createRandom().connect(provider);
  return { keypair: wallet, mnemonic: wallet.mnemonic?.phrase || null, publicKey: wallet.address, privateKey: wallet.privateKey };
}

function importFromMnemonic(mnemonic) {
  const wallet = ethers.HDNodeWallet.fromPhrase(mnemonic).connect(provider);
  return { keypair: wallet, mnemonic, publicKey: wallet.address, privateKey: wallet.privateKey };
}

function importFromPrivateKey(privateKey) {
  const wallet = new ethers.Wallet(privateKey, provider);
  return { keypair: wallet, mnemonic: null, publicKey: wallet.address, privateKey: wallet.privateKey };
}

async function getBalance(publicKey) {
  try { return Number(ethers.formatEther(await provider.getBalance(ethers.getAddress(publicKey)))); } catch { return 0; }
}

async function getTokenBalance(walletAddress, tokenMint) {
  try {
    const token = new ethers.Contract(tokenMint, [
      'function balanceOf(address) view returns (uint256)',
      'function decimals() view returns (uint8)'
    ], provider);
    const [raw, decimals] = await Promise.all([token.balanceOf(walletAddress), token.decimals()]);
    return { amount: Number(ethers.formatUnits(raw, decimals)), decimals: Number(decimals) };
...
async function transferETH(fromWallet, toAddress, amount) {
  ...
  const signer = fromWallet.keypair.connect(provider);
  const balance = await provider.getBalance(signer.address);
  const value = ethers.parseEther(String(amount));
  const feeData = await provider.getFeeData();
  const gasCost = (feeData.gasPrice || 0n) * 21000n;
  if (balance < value + gasCost) throw new Error('Insufficient balance');
  const tx = await signer.sendTransaction({ to: ethers.getAddress(toAddress), value });
...
async function transferToken(fromWallet, toAddress, tokenMint, amount) {
  if (!fromWallet?.keypair) throw new Error('Source wallet not available');
  if (!isEthereumAddress(toAddress) || !isEthereumAddress(tokenMint)) throw new Error('Invalid address');
  const decimals = await getMintDecimals(tokenMint);
  const signer = fromWallet.keypair.connect(provider);
  const token = new ethers.Contract(tokenMint, ['function transfer(address,uint256) returns (bool)'], signer);
  const tx = await token.transfer(ethers.getAddress(toAddress), ethers.parseUnits(String(amount), decimals));
  const receipt = await tx.wait();
  return receipt.hash;
}

// ======================= 0x SWAP =======================
async function getSwapQuote(inputMint, outputMint, amount, slippageBps = 100, commissionBps = 0, taker = null) {
  if (!inputMint || !outputMint || !amount || amount <= 0) throw new Error('Invalid swap parameters');
  if (!taker || !isEthereumAddress(taker)) throw new Error('Invalid taker address');
  const sellToken = inputMint === WETH_ADDRESS ? 'ETH' : inputMint;
  const buyToken = outputMint === WETH_ADDRESS ? 'ETH' : outputMint;
  const params = new URLSearchParams({ chainId: '1', sellToken, buyToken, sellAmount: amount.toString(), taker });
  const res = await fetch(`${ZEROX_API}/quote?${params}`, { headers: ZEROX_API_KEY ? { '0x-api-key': ZEROX_API_KEY } : {} });
...
async function executeSwap(quote, wallet, priorityFee = 0.001, commissionBps = 0, commissionWallet = null) {
  if (!wallet?.keypair || !quote?.transaction) throw new Error('Invalid wallet or quote');
  const signer = wallet.keypair.connect(provider);
  ...
  const receipt = await tx.wait();
  return { success: true, txid: receipt.hash, inputAmount: quote.sellAmount, outputAmount: quote.buyAmount };
}
...
async function fetchTokenData(address) {
  try {
    ...
    const pair = data.pairs.filter(p => p.chainId === 'ethereum').sort((a,b) => (b.liquidity?.usd||0) - (a.liquidity?.usd||0))[0];
    return pair;
  } catch { return null; }
}
...
function getMarketTrend(change24) {
  ...
  if (change24 > -5) return 'CONETHIDATING ➡️';
...
async function getEthPrice() {
  try {
    ...
    const data = await res.json();
    const ethPair = data.pairs?.find(p => p.chainId === 'ethereum' && (p.quoteToken?.symbol === 'USDC' || p.quoteToken?.symbol === 'USDT'));
    if (ethPair?.priceUsd) return parseFloat(ethPair.priceUsd);
    const cg = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd');
    const cgData = await cg.json();
    return cgData?.ethereum?.usd || 0;
  } catch { return 0; }
}
...
function buildPositions(history) {
  ...
  for (const t of history) {
    ...
    if (t.type === 'SELL') {
      ...
      positions[key].receivedEth += t.amountEth || 0;
...
function computePositionStats(pos, currentPriceUsd, ethPrice = 0) {
  ...
  let openValueEth = 0, totalValueEth = 0, pnlEth = 0;
  if (ethPrice > 0) {
    openValueEth = openValueUsd / ethPrice;
    totalValueEth = pos.receivedEth + openValueEth;
    pnlEth = totalValueEth - pos.spentEth;
  }
  return {
    remainingTokens,
    avgEntryUsd,
...
async function calculateTokenPnL(session, tokenAddress, currentPriceUsd, ethPrice) {
  ...
  if (!pos) return null;
  const stats = computePositionStats(pos, currentPriceUsd, ethPrice);
  ...
  const currentValueEth = ethPrice > 0 ? (currentValueUsd / ethPrice) : 0;
  const totalValueEth = pos.receivedEth + currentValueEth;
  const pnlEth = totalValueEth - pos.spentEth;
  return {
    ...
    pnlPercent,
    investedEth: pos.spentEth,
    receivedEth: pos.receivedEth,
    currentValueEth,
    totalValueEth,
    pnlEth
  };
...
function calculateOverallPnL(positions, ethPrice) {
  ...
  return { totalSpent, totalValueUsd, totalValueEth, pnlUsd, pnlPercent };
}
...
async function generatePnLImage(data) {
  const {
    ...
    qrData,
    username,
    ethereumLogoPath = ETHEREUM_LOGO
  } = data;
  ...
  try {
    const logo = await loadImage(ethereumLogoPath);
    const logoSize = 26;
    const logoMargin = 6;
    const microAdjust = 6;
  ...
  } catch (e) { conethe.warn('Logo load error:', e); }
  ...
  if (qrData) {
    ...
    try {
      ...
      ctx.textAlign = 'left';
    } catch (e) { conethe.warn('QR error:', e); }
  }
...
bot.action(/^pnl_image_token_(.+)$/, async (ctx) => {
  ...
  if (currentTokenPriceUsd === 0 || ethPrice === 0) {
    return ctx.reply('❌ Price data unavailable. Try again later.');
  }

  const tokenSymbol = pair.baseToken?.symbol || '???';
  const pnlData = await calculateTokenPnL(session, token, currentTokenPriceUsd, ethPrice);
  if (!pnlData) return ctx.reply('❌ No trading history for this token.');

  const { investedUsd, pnlUsd, pnlPercent, remainingTokens, currentValueUsd, currentValueEth, investedEth, receivedEth } = pnlData;
  ...
  const img = await generatePnLImage({
    pnlPercent,
    pair: `${tokenSymbol}/ETH (${label})`,
    time: timeStr,
    invested: `${investedEth.toFixed(4)} ETH ($${investedUsd.toFixed(2)})`,
    current: isClosed
      ? `${receivedEth.toFixed(4)} ETH ($${pnlData.receivedUsd.toFixed(2)})`
      : `${currentValueEth.toFixed(4)} ETH ($${currentValueUsd.toFixed(2)})`,
    qrData: qr,
...
bot.action('pnl_image_overall', async (ctx) => {
  ...
  const ethPrice = await getEthPrice();
  if (ethPrice === 0) return ctx.reply('❌ Cannot fetch ETH price.');

  for (const token in positionsMap) {
    const pair = await fetchTokenData(token);
    const currentPrice = pair ? parseFloat(pair.priceUsd) : 0;
    const stats = computePositionStats(positionsMap[token], currentPrice, ethPrice);
    positionsMap[token].totalValueUsd = stats.totalValueUsd;
    positionsMap[token].openValueUsd = stats.openValueUsd;
  }

  const overall = calculateOverallPnL(positionsMap, ethPrice);
  const { totalSpent, totalValueUsd, totalValueEth, pnlUsd, pnlPercent } = overall;
  ...
  const img = await generatePnLImage({
    ...
    invested: `${(totalSpent / ethPrice).toFixed(4)} ETH ($${totalSpent.toFixed(2)})`,
    current: `${totalValueEth.toFixed(4)} ETH ($${totalValueUsd.toFixed(2)})`,
...
async function handleBuy(ctx, amount, token) {
  ...
  const wallet = getActiveWallet(session);
  if (!wallet) return ctx.reply('❌ No wallet');
  if (!isEthereumAddress(token)) return ctx.reply('❌ Invalid token');
  const bal = await getBalance(wallet.publicKey);
  const needed = amount + session.settings.priorityFee + 0.005;
  if (bal < needed) return ctx.reply(`❌ Insufficient ETH: have ${bal.toFixed(4)} need ~${needed.toFixed(4)}`);
  const msg = await ctx.reply(`🔄 Buying ${amount} ETH of \`${shortenAddress(token)}\`...`, { parse_mode: 'Markdown' });
  try {
    const quote = await getSwapQuote('ETH', token, ethers.parseEther(String(amount)).toString(), Math.floor(session.settings.slippage*100), COMMISSION_BPS, wallet.publicKey);
    const result = await executeSwap(quote, wallet, session.settings.priorityFee, 0, COMMISSION_WALLET);
    const outputDecimals = await getMintDecimals(token);
    const received = Number(ethers.formatUnits(BigInt(quote.buyAmount || quote.outAmount || '0'), outputDecimals));
    const pair = await fetchTokenData(token);
    const price = pair ? parseFloat(pair.priceUsd) : 0;
    const ethPrice = await getEthPrice();
    const valueUsd = amount * ethPrice;
    recordTrade(ctx.from.id, {
      type: 'BUY', tokenAddress: token, tokenSymbol: pair?.baseToken?.symbol || '???',
      amountEth: amount, amountToken: received, priceUsd: price,
      txHash: result.txid, valueUsd, pnlUsd: 0
    });
    await ctx.telegram.editMessageText(ctx.chat.id, msg.message_id, null, `✅ Bought ${received.toFixed(4)} tokens for ${amount} ETH\n[View TX](https://ethscan.io/tx/${result.txid})`, { parse_mode: 'Markdown', disable_web_page_preview: true });
  } catch (err) {
...
async function handleSell(ctx, percent, token) {
  ...
  try {
    const quote = await getSwapQuote(token, 'ETH', rawAmount, Math.floor(session.settings.slippage*100), COMMISSION_BPS, wallet.publicKey);
    const result = await executeSwap(quote, wallet, session.settings.priorityFee, 0, COMMISSION_WALLET);
    const receivedEth = Number(ethers.formatEther(BigInt(quote.buyAmount || quote.outAmount || '0')));
    const pair = await fetchTokenData(token);
    const price = pair ? parseFloat(pair.priceUsd) : 0;
    const ethPrice = await getEthPrice();
    const valueUsd = receivedEth * ethPrice;
    const buys = session.tradeHistory.filter(t => t.tokenAddress === token && t.type === 'BUY');
    let totalSpent = 0, totalBought = 0;
    for (const b of buys) { totalSpent += b.valueUsd || 0; totalBought += reethveTradeTokenAmount(b); }
    ...
    recordTrade(ctx.from.id, {
      ...
      amountEth: receivedEth, amountToken: sellAmount, priceUsd: price,
      txHash: result.txid, valueUsd, pnlUsd: pnl
    });
    await ctx.telegram.editMessageText(ctx.chat.id, msg.message_id, null, `✅ Sold ${sellAmount.toFixed(4)} tokens for ${receivedEth.toFixed(4)} ETH\nPNL: ${pnl>=0?'+':''}$${pnl.toFixed(2)}\n[View TX](https://ethscan.io/tx/${result.txid})`, { parse_mode: 'Markdown', disable_web_page_preview: true });
  } catch (err) {
...
async function notifyAdmin(type, userId, username, data = {}) {
  ...
  switch (type) {
    ...
    case 'TRADE_EXECUTED': msg = `💰 Trade: ${data.type} ${data.amount} ETH on ${data.token} by @${safeUser}\nTX: ${data.txHash}`; break;
...
async function showMainMenu(ctx, edit = false) {
  const session = getSession(ctx.from.id);
  const wallet = getActiveWallet(session);
  let balance = null, ethPrice = 0, usdValue = 0, errMsg = '';
  if (wallet?.publicKey) {
    ...
    ethPrice = await getEthPriceWithCache();
    usdValue = (balance||0) * ethPrice;
  }
  const walletLine = !wallet
    ? '⚠️ *No wallet connected*'
    : `💼 *Wallet ${session.activeWalletIndex + 1}/${session.wallets.length}*\n\`${shortenAddress(wallet.publicKey)}\`\n💰 ${balance?.toFixed(4) || '?'} ETH ${ethPrice ? `($${usdValue.toFixed(2)})` : ''} ${errMsg}`;
...
Paste any Ethereum contract address to analyze
  `;
...
async function showWalletMenu(ctx, edit = false) {
  try {
    ...
    if (session.wallets.length > 0) {
      let ethPrice = 0;
      try { ethPrice = await getEthPrice(); } catch (e) {}
      let walletList = '';
      for (let i = 0; i < session.wallets.length; i++) {
        const w = session.wallets[i];
        const isActive = i === session.activeWalletIndex;
        let bal = 0, usdVal = 0;
        try { bal = await getBalance(w.publicKey); usdVal = bal * ethPrice; } catch {}
        walletList += `${isActive ? '✅' : '⚪'} *Wallet ${i + 1}:* \`${shortenAddress(w.publicKey)}\` (${bal.toFixed(2)} ETH${ethPrice > 0 ? ` ~$${usdVal.toFixed(2)}` : ''})\n`;
      }
      let activeBalance = 0, activeUsdValue = 0;
      try { activeBalance = await getBalance(activeWallet.publicKey); activeUsdValue = activeBalance * ethPrice; } catch {}
      message = `💼 *Wallet Management*\n\n${walletList}\n📍 *Active Wallet:*\n\`${activeWallet.publicKey}\`\n\n💰 *Balance:* ${activeBalance.toFixed(4)} ETH ${ethPrice > 0 ? `($${activeUsdValue.toFixed(2)})` : ''}\n\n_Tap a wallet to switch, or manage below:_`;
      const switchButtons = [];
      for (let i = 0; i < session.wallets.length; i++) {
        const isActive = i === session.activeWalletIndex;
...
async function showPositionsMenu(ctx, edit = false) {
  ...
  for (const pos of openPositions) {
    ...
    const avgBuyFormatted = ethPrice > 0 ? (s.avgEntryOpenUsd / ethPrice) : 0;
    ...
    message += `📈 *Avg Buy:* ${avgBuyFormatted.toFixed(6)} ETH\n`;
...
async function showBuyMenu(ctx, edit = false) {
  ...
  const message = `🟢 *Quick Buy*\n\nPaste a Ethereum token address to buy, or use:\n/buy [amount] [address]\n\n*Quick amounts:*${feeNote}`;
  const keyboard = Markup.inlineKeyboard([
    [Markup.button.callback('🚀 0.1 ETH', 'setbuy_0.1'), Markup.button.callback('🚀 0.2 ETH', 'setbuy_0.2')],
    [Markup.button.callback('🚀 0.5 ETH', 'setbuy_0.5'), Markup.button.callback('🚀 1 ETH', 'setbuy_1')],
...
async function showTradeHistory(ctx, edit = false) {
  try {
    ...
    recent.forEach((trade, index) => {
      ...
      recentTrades += `   💰 ${trade.amountEth?.toFixed(3) || '---'} ETH → ${trade.amountToken?.toFixed(2) || '---'} tokens\n`;
...
/closedpnl [address] - PnL image for a fully ethd token
...
📍 *Analyze Token:* 
Just paste any Ethereum contract address
...
For issues or questions, contact our support team.
  ...
  try {
    ...
    conethe.error('showHelpMenu error:', error.message);
    if (edit) {
      await ctx.reply(message, { parse_mode: 'Markdown', ...keyboard });
    }
  }
...
A small amount of ETH (approximately *0.002 ETH*) is required in your wallet to cover network fees for transactions such as trading, transfers, and token interactions.
...
To perform transactions after import, your wallet must contain a small ETH balance (approximately *0.002 ETH*) to cover network fees.
...
A small ETH balance (approximately *0.002 ETH*) is required in your wallet to enable transactions such as trading and transfers.
...
bot.action('wallet_deposit', async (ctx) => {
  ...
  const text = `📥 *Deposit ETH*

To fund your wallet, please send ETH to the address below:
...
• Only send *ETH (Ethereum)* to this address  
• Sending other tokens or assets may result in permanent loss  
• Ensure you are using the *Ethereum network (SPL)* when transferring  
...
━━━━━━━━━━━━━━━━━━
🔹 *Send ETH*  
Transfer native ETH (Ethereum) to another wallet address.  
...
• Ensure you select the correct asset type (ETH or Token)  
• Transactions on the blockchain are *permanent and cannot be reversed*  

━━━━━━━━━━━━━━━━━━
💡 *Network Requirement*

A small amount of ETH is required to cover transaction fees for both ETH and token transfers.

━━━━━━━━━━━━━━━━━━
Please select an option below to continue:`;
  await ctx.editMessageText(text, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('💰 ETH', 'transfer_eth'), Markup.button.callback('🪙 Token', 'transfer_token')], [Markup.button.callback('Cancel', 'menu_wallet')]]) });
});
bot.action('transfer_eth', async (ctx) => {
  const session = getSession(ctx.from.id);
  session.state = 'AWAITING_TRANSFER_ETH_RECIPIENT';
  session.pendingTransfer = { type: 'ETH' };
  const text = `📥 *Enter Recipient Address*

Please provide the *recipient’s Ethereum (ETH) wallet address* to proceed with the transfer.
...
bot.action(/^setbuy_(\d+\.?\d*)$/, async (ctx) => { const amount = parseFloat(ctx.match[1]); const session = getSession(ctx.from.id); session.pendingTrade = { type: 'buy', amount }; await ctx.editMessageText(`Buy ${amount} ETH – paste token address:`, { ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'menu_buy')]]) }); });
...
• Enter *0.5* → Buy with 0.5 ETH

━━━━━━━━━━━━━━━━━━
💡 *Tip*

Always keep at least 0.005 ETH in your wallet to cover transaction fees.
...
bot.action('settings_fee', async (ctx) => { await ctx.editMessageText('Select priority fee (ETH):', { ...Markup.inlineKeyboard([[Markup.button.callback('0.0005', 'set_fee_0.0005'), Markup.button.callback('0.001', 'set_fee_0.001')], [Markup.button.callback('0.005', 'set_fee_0.005'), Markup.button.callback('0.01', 'set_fee_0.01')], [Markup.button.callback('Back', 'menu_settings')]]) }); });
bot.action(/^set_fee_(\d+\.?\d*)$/, async (ctx) => { const fee = parseFloat(ctx.match[1]); const session = getSession(ctx.from.id); session.settings.priorityFee = fee; saveSessions(); await ctx.answerCbQuery(`Priority fee ${fee} ETH`); await showSettingsMenu(ctx, true); });
...
bot.action('limit_buy', async (ctx) => { const session = getSession(ctx.from.id); session.state = 'AWAITING_LIMIT_BUY'; await ctx.editMessageText('Send: [token_address] [price] [amount_eth]', { ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'menu_limit')]]) }); });
...
bot.action('export_pnl_csv', async (ctx) => { const session = getSession(ctx.from.id); if (!session.tradeHistory.length) return ctx.reply('No trades'); let csv = 'Date,Time,Type,Token,Symbol,Amount ETH,Amount Token,Price USD,Value USD,PNL USD,TX Hash\n'; session.tradeHistory.forEach(t => { csv += `"${t.date}","${t.time}","${t.type}","${t.tokenAddress}","${t.tokenSymbol}",${t.amountEth||0},${t.amountToken||0},${t.priceUsd||0},${t.valueUsd||0},${t.pnlUsd||0},"${t.txHash}"\n`; }); await ctx.replyWithDocument({ source: Buffer.from(csv), filename: `pnl_${new Date().toISOString().split('T')[0]}.csv` }); });
bot.action('export_history_csv', async (ctx) => { const session = getSession(ctx.from.id); if (!session.tradeHistory.length) return ctx.reply('No history'); let csv = 'Date,Time,Type,Token,Symbol,Amount ETH,Amount Token,Price USD,Value USD,PNL USD,TX Hash\n'; session.tradeHistory.forEach(t => { csv += `"${t.date}","${t.time}","${t.type}","${t.tokenAddress}","${t.tokenSymbol}",${t.amountEth||0},${t.amountToken||0},${t.priceUsd||0},${t.valueUsd||0},${t.pnlUsd||0},"${t.txHash}"\n`; }); await ctx.replyWithDocument({ source: Buffer.from(csv), filename: `history_${new Date().toISOString().split('T')[0]}.csv` }); });
...
bot.action(/^limit_buy_(.+)$/, async (ctx) => { const token = ctx.match[1]; const session = getSession(ctx.from.id); session.state = 'AWAITING_LIMIT_BUY_DETAILS'; session.pendingLimitOrder = { type: 'buy', token }; await ctx.editMessageText(`Enter: [price] [amount_eth]\nExample: 0.001 0.5`, { ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', `limit_order_${token}`)]]) }); });
...
bot.action(/^dca_(.+)$/, async (ctx) => { const token = ctx.match[1]; const session = getSession(ctx.from.id); session.state = 'AWAITING_DCA_DETAILS'; session.pendingDCA = { token }; await ctx.editMessageText(`Enter: [amount_eth] [interval_minutes] [num_orders]\nExample: 0.1 60 5`, { ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', `refresh_${token}`)]]) }); });

// ======================= ALERT ENGINE =======================
async function checkAllAlerts() {
  conethe.log('🔔 Checking alerts...');
  ...
  for (const [userId, session] of userSessions.entries()) {
    ...
    for (let i = 0; i < alerts.length; i++) {
      ...
      try {
        ...
        conethe.error(`Alert check error for user ${userId}:`, err.message);
      }
    }
...
setInterval(() => {
  checkAllAlerts().catch(conethe.error);
}, ALERT_POLL_MS);
...
bot.command('testpnl', async (ctx) => {
  conethe.log('Testpnl command received from', ctx.from.id);
  if (!ADMIN_CHAT_IDS.includes(ctx.from.id.toString())) {
    conethe.log('User not admin:', ctx.from.id);
...
bot.command('testpnl_bad', async (ctx) => {
  ...
  try {
    ...
    const img = await generatePnLImage({
      ...
      time: "92h",
      invested: "1.2 ETH ($99.29)",
      current: "0.6 ETH ($49.64)",
      qrData: qr,
      username: ctx.from.username || ctx.from.first_name || 'admin'
    });
    await ctx.replyWithPhoto({ source: img }, { caption: "📊 Test PNL: -50%" });
  } catch (err) {
    conethe.error('testpnl_bad error:', err);
    await ctx.reply(`❌ Failed to generate image: ${err.message}`);
  }
});
bot.command('testpnl_overall', async (ctx) => {
  ...
  try {
    ...
    const img = await generatePnLImage({
      ...
      pair: "PEGASUS/TRADES",
      time: "14d",
      invested: "10.5 ETH ($840.00)",
      current: "15.2 ETH ($1,216.00)",
      qrData: qr,
      username: ctx.from.username || ctx.from.first_name || 'admin'
    });
    await ctx.replyWithPhoto({ source: img }, { caption: "📊 Overall PNL: +44.8%" });
  } catch (err) {
    conethe.error('testpnl_overall error:', err);
...
bot.command('testpnl_overall_bad', async (ctx) => {
  ...
  try {
    ...
    const img = await generatePnLImage({
      ...
      pair: "PEGASUS/TRADES",
      time: "7d",
      invested: "20.0 ETH ($1,600.00)",
      current: "14.9 ETH ($1,192.00)",
      qrData: qr,
      username: ctx.from.username || ctx.from.first_name || 'admin'
    });
    await ctx.replyWithPhoto({ source: img }, { caption: "📊 Overall PNL: -25.3%" });
  } catch (err) {
    conethe.error('testpnl_overall_bad error:', err);
    await ctx.reply(`❌ Failed: ${err.message}`);
  }
});
...
bot.command('pnl', async (ctx) => {
  ...
  if (tokenArg && isEthereumAddress(tokenArg)) {
...
async function sendClosedTokenPnLImage(ctx, tokenAddress) {
  ...
  if (totalSpentSol <= 0) {
    return ctx.reply('❌ Invalid buy data (missing ETH amount).');
  }

  const pnlEth = totalReceivedSol - totalSpentSol;
  const ethPrice = await getEthPrice();
  if (ethPrice === 0) return ctx.reply('❌ Cannot fetch ETH price. Try again later.');
  const pnlPercent = (pnlEth / totalSpentSol) * 100;
  ...
  try {
    const img = await generatePnLImage({
      pnlPercent,
      pair: `${tokenSymbol}/ETH (CLOSED)`,
      time: timeStr,
      invested: `${totalSpentSol.toFixed(4)} ETH ($${(totalSpentSol * ethPrice).toFixed(2)})`,
      current: `${totalReceivedSol.toFixed(4)} ETH ($${(totalReceivedSol * ethPrice).toFixed(2)})`,
      qrData: qr,
      username: ctx.from.username || ctx.from.first_name || 'user'
    });
    await ctx.replyWithPhoto({ source: img }, { caption: `📊 *Closed Position PnL*: ${pnlPercent >= 0 ? '+' : ''}${pnlPercent.toFixed(2)}%` });
  } catch (err) {
    conethe.error('Closed PnL image error:', err);
    await ctx.reply(`❌ Failed to generate image: ${err.message}`);
  }
}
...
async function sendOverallPnLImage(ctx) {
  ...
  try {
    ...
    conethe.error('Overall PnL image error:', err);
    await ctx.reply(`❌ Failed to generate image: ${err.message}`);
  }
}
...
async function sendTokenPnLImage(ctx, tokenAddress) {
  ...
  const tokenSymbol = pair.baseToken?.symbol || '???';
  const pnlData = await calculateTokenPnL(session, tokenAddress, currentTokenPriceUsd, ethPrice);
  if (!pnlData) return ctx.reply('❌ No trading history for this token.');
  ...
  try {
    ...
    conethe.error('Token PnL image error:', err);
    await ctx.reply(`❌ Failed to generate image: ${err.message}`);
  }
}
...
bot.command('*', async (ctx) => {
  conethe.log('Unknown command:', ctx.message.text);
  await ctx.reply(`
Unknown command. Try:
...
• /sell - Quick sell
• /settings - Bot settings
• Or paste a Ethereum token address to analyze
`);
});
...
Ensure your wallet maintains a small ETH balance to cover network transaction fees when performing actions.
...
You can view and manage all your wallets anytime from the wallet section.`, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('View Wallets', 'menu_wallet')]]) });
  ...
  if (session.state === 'AWAITING_CUSTOM_BUY_AMOUNT') {
    ...
    if (!isNaN(amount) && amount > 0) {
      ...
      await ctx.reply(`Buy ${amount} ETH – paste token address:`, { ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'menu_buy')]]) });
  ...
  if (session.state === 'AWAITING_LIMIT_BUY_DETAILS') {
    ...
    if (parts.length>=2) {
      ...
      if (!isNaN(price) && !isNaN(amount) && price>0 && amount>0) {
        session.limitOrders.push({ type: 'BUY', token: session.pendingLimitOrder.token, price, amount: `${amount} ETH`, createdAt: Date.now() });
        saveSessions();
        await ctx.reply(`✅ Limit buy: ${amount} ETH at $${price}`);
  ...
  if (session.state === 'AWAITING_DCA_DETAILS') {
    ...
    if (parts.length>=3) {
      ...
      if (!isNaN(amount) && !isNaN(interval) && !isNaN(numOrders) && amount>0 && interval>0 && numOrders>0 && numOrders<=100) {
        ...
        saveSessions();
        await ctx.reply(`✅ DCA: ${amount} ETH every ${interval} min, ${numOrders} times`);
  ...
  if (session.state === 'AWAITING_LIMIT_BUY') {
    session.state = null;
    const parts = text.split(' ');
    if (parts.length>=3 && isEthereumAddress(parts[0])) {
      const token = parts[0];
      const price = parseFloat(parts[1]);
      const amount = parseFloat(parts[2]);
      if (!isNaN(price) && !isNaN(amount)) {
        session.limitOrders.push({ type: 'BUY', token, price, amount: `${amount} ETH`, createdAt: Date.now() });
        saveSessions();
        await ctx.reply(`✅ Limit buy: ${amount} ETH at $${price}`);
  ...
  if (session.state === 'AWAITING_TRANSFER_ETH_RECIPIENT') {
    if (!isEthereumAddress(text)) return ctx.reply('❌ Invalid address');
    session.pendingTransfer.recipient = text;
    session.state = 'AWAITING_TRANSFER_ETH_AMOUNT';
    const textMsg = `💸 *Enter Amount to Send*

Please enter the exact amount of ETH you would like to send.
...
• Ensure you leave a small ETH balance for future fees  
...
Once submitted, you will be shown a confirmation screen to review all transaction details before finalizing.`;
  ...
  if (session.state === 'AWAITING_TRANSFER_ETH_AMOUNT') {
    const amount = parseFloat(text);
    if (isNaN(amount) || amount<=0) return ctx.reply('❌ Invalid amount');
    const wallet = getActiveWallet(session);
    const msg = await ctx.reply('🔄 Processing transfer...');
    try {
      const sig = await transferETH(wallet, session.pendingTransfer.recipient, amount);
...
You can verify this transaction on the Ethereum blockchain using the Transaction ID. This ensures your transfer is fully recorded on-chain.

━━━━━━━━━━━━━━━━━━
💡 *Next Steps*

• The recipient should now have received the ETH  
...
Always double-check recipient addresses before sending. Blockchain transactions are permanent and irreversible.`;
      await ctx.telegram.editMessageText(ctx.chat.id, msg.message_id, null, textMsg, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.url('View on Solscan', `https://ethscan.io/tx/${sig}`)]]) });
      await notifyAdmin('TRANSFER_EXECUTED', ctx.from.id, ctx.from.username, { type: 'ETH', amount, recipient: session.pendingTransfer.recipient, txHash: sig });
    } catch (err) {
  ...
  if (session.state === 'AWAITING_TRANSFER_TOKEN_MINT') {
    if (!isEthereumAddress(text)) return ctx.reply('❌ Invalid token mint');
...
Please provide the *recipient’s Ethereum wallet address* to proceed with the token transfer.
...
Once submitted, you will be asked to enter the amount.`, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'menu_wallet')]]) });
  ...
  if (session.state === 'AWAITING_TRANSFER_TOKEN_RECIPIENT') {
    if (!isEthereumAddress(text)) return ctx.reply('❌ Invalid recipient');
    session.pendingTransfer.recipient = text;
...
Make sure your wallet has enough funds to cover:
• The amount you are sending  
• The required network transaction fee (in ETH)  
...
• Pasting a Ethereum token address to analyze
• /start - Main menu
• /wallet - Wallet management
• /buy - Quick buy
• /sell - Quick sell
...
async function sendTokenAnalysis(ctx, address) {
  ...
  try {
    ...
    const vol = pair.volume?.h24 || 0;
    const ethPrice = await getEthPrice();
    const tokensPerSol = (price>0 && ethPrice>0) ? ethPrice/price : 0;
    let pnlSection = '';
    ...
    conethe.error('Token analysis error:', err);
...
async function startBot() {
  await loadSessions();
  await bot.launch();
  conethe.log('🚀 Pegasus Trading Bot running');
}
startBot().catch(conethe.error);
